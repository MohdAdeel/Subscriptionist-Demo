...
async function exportToPDF() {
  ...
  all = all.filter((subscription, index, self) =>
  ...
  await html2pdf().from(exportContainer).set({
    margin: 0.5,
...
async function exportToExcel() {
  ...
  const toSheetData = (arr) =>
    arr.map(s => ({
      ...
      'Start Date': s.SubscriptionStartDate ? new Date(s.SubscriptionStartDate).toLocaleDateString() : '',
...
const section = document.createElement('div');
  ...
  exportContainer.innerHTML += buildSection(
    ...
    s => new Date(s.SubscriptionEndDate) > today &&
         new Date(s.SubscriptionStartDate) > today
  );
...
const nearing = nearingRenewal.filter(s =>
  new Date(s.SubscriptionEndDate) > today &&
  new Date(s.SubscriptionStartDate) > today
).filter((s, index, self) =>
...
const expired = nearingRenewal.filter(s =>
  new Date(s.SubscriptionEndDate) <= today
).filter((s, index, self) =>
  index === self.findIndex(sub =>
    sub.SubscriptionName === s.SubscriptionName &&
...
openPopup("popup_loading");
  ...
  if (canvas) {
    const titleElement = canvas.closest('.page-dashboard__listQuickStart-item')?.querySelector('p');
  ...
  // 4. Append tables
  exportContainer.innerHTML += buildSectionDepartment('Spend By Department', dataOfDepartmentForShowingInPagination);
  exportContainer.innerHTML += buildSectionCategory('Spend By Category', dataOfCategoryForPagination);
...
for (let row = 1; row <= 150; row++) {
  ...
  const chartRows = [
    [ // Row group 1
      { col: 1, chartId: 'chart5', legendId: null, title: 'Spend by Department', width: 950, height: 320 },
      { col: 15, chartId: 'doughnut-chart', legendId: 'chart-legend', title: 'Spend by Vendor', width: 320, height: 320 }
    ],
    [ // Row group 2
      { col: 1, chartId: 'line-chart', legendId: null, title: 'Spend by Subscription type', width: 1400, height: 360 }
    ],
    [ // Row group 3
      { col: 1, chartId: 'gradient-chart', legendId: null, title: 'Budget vs Actual Report', width: 1400, height: 360 }
    ],
    [ // Row group 4
      { col: 1, chartId: 'most-expensive-chart', legendId: null, title: 'Most Expensive Subscriptions', width: 950, height: 320 },
      { col: 15, chartId: 'usage-analysis-chart', legendId: 'chart-legend-analysis', title: 'Usage Analysis', width: 320, height: 320 }
    ]
  ];
...
if (chart.legendId) {
  ...
  const deptData = dataOfDepartmentForShowingInPagination.map(group => {
    ...
    return {
      ...
      budget: item?.DepartmentNames?.Budget ? `$${Number(item.DepartmentNames.Budget).toFixed(2)}` : '$0.00'
    };
  });
...
function setMostExpensiveChart(expensiveSubscriptions) {
  ...
  const departmentNames = hasData ? expensiveSubscriptions.map(sub => sub.SubscriptionName) : ["No Data Available"];
  const departmentAmounts = hasData ? expensiveSubscriptions.map(sub => sub.SubscriptionContractAmount.Value) : [0];
...
function aggregateSubscriptionsByCategory(subscriptions) {
    ...
    const filteredSubscriptions = result.filter(subscription => {
      ...
      const isWithinRange = (filtersForFinance.amount1 === null || amountValue >= filtersForFinance.amount1) &&
  ...
  function setGradientChart(data) {
    ...
    const budgetData = flattenedData.map(record =>
      record.DepartmentNames && record.DepartmentNames.Budget !== null ?
        (record.DepartmentNames.Budget || 0) : 0
    );
  ...
  $(function () {
    ...
    function initializeDateRangePicker(inputName) {
      $('input[name="' + inputName + '"]').daterangepicker({
        ...
        $('input[name="' + inputName + '"]').val(start.format('DD.MM.YYYY') + ' to ' + end.format('DD.MM.YYYY'));
      });
...
function resetSlider(rangeInputs, range, minLabel, maxLabel) {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (!popoverContent.contains(event.target) && !filterButton.contains(event.target) && !isDatePickerOpen) {
      // Hide the popover
      popoverContent.style.display = "none";
    }
    ...
    // Check if the click was outside the popover and the button
    if (!popoverContentFinance.contains(event.target) && !filterButtonFinance.contains(event.target) && !isDatePickerOpen) {
      // Hide the popover
    ...
    // Check if the click was outside the popover and the button
    if (!popoverContentRenwel.contains(event.target) && !filterButtonRenwel.contains(event.target) && !isDatePickerOpen) {
      // Hide the popover
  ...
  function updateDepartmentChart(DepartmentData) {
    ...
    if (hasValidData) {
      departmentNames = DepartmentData.map(department =>
        department[0]?.DepartmentNames?.Name || "Unknown"
      );

      departmentAmounts = DepartmentData.map(department =>
        department[0]?.SubscriptionContractAmount?.Value || 0
      );
...
//restting all the variables to onload state
   ...
   //remove duplicated from array  
    ...
    if (status !== null) {
      loadMoreArray = loadMoreArray.filter(subscription => {
        ...
        if (status === 0) { // status shows on which tab is user write now
  ...
  function generateSimilarRecordsByYearForDepartment(record) {
    ...
    if (RenewalDate >= startDate && RenewalDate < endDate) {
      while ((startDate.getFullYear() < endDateForComparison.getFullYear())) {
        var subscriptionAmount = record.SubscriptionContractAmount !== null ? record.SubscriptionContractAmount.Value : 0;
  ...
  function generateSimilarRecordsbyMonthforDepartment(record) {
    ...
    if (RenewalDate >= startDate && RenewalDate < endDate) {
      while ((startDate < endDateForComparison)) {
        ...
        if (startDate.getFullYear() === endDateForComparison.getFullYear() && startDate.getMonth() === endDateForComparison.getMonth()) {
  ...
  function processVendorProfiles() {
    ...
    vendorProfileCounts = Array.from(profileMap, ([VendorProfile, count]) => ({ VendorProfile, count }));
  ...
  var departmentSliderValue = new Date(new Date().getFullYear() + 1, new Date().getMonth(), new Date().getDate());
...
function handleActivityLinesSuccess(originalData, accountId, ilojcDiv) {
    try {
        ...
        if (originalData && Array.isArray(originalData)) {
            ...
            console.log("Found value array (OData format), length:", lines.length);
        } else {
            ...
            if (arrayKeys.length > 0) {
                ...
                console.log(`Using first array property "${arrayKeys[0]}", length:`, lines.length);
            } else {
        ...
        console.log("Processing", lines.length, "activity lines");

        // Transform Azure Function response
        const transformedLines = transformAzureResponseToBlobFormat(lines);
        console.log("Transformed", transformedLines.length, "lines");
        ...
        return azureLines.map((line, index) => {
            try {
                ...
                const transformedLine = {
                    ...
                    Amount: line.SubscriptionContractAmount?.Value || 
                           line.Amount || 
                           line.SubscriptionAmount || 
                           0,
                    
                    // Add any other fields that existing functions expect
                    DepartmentName: line.DepartmentNames?.Name || 
                                   line.DepartmentName,
                    DepartmentBudget: line.DepartmentNames?.Budget?.Value || 
                                     line.DepartmentBudget
                };
    ...
    SubscriptionJSon.forEach(function (subArray) {
        ...
        subArray.forEach(function (record) {
            ...
            if (record.SubscriptionFrequency && record.SubscriptionFrequency.trim() !== "") {
                ...
                if (record.SubscriptionFrequency.includes('Monthly') || record.SubscriptionFrequency.includes('Months')) {
                ...
                else if (record.SubscriptionFrequency.includes('Yearly') || record.SubscriptionFrequency.includes('Years')) {
...
function fillExpiredSubscriptionsTable(subscriptions) {
    ...
    const recordsToShow = filteredSubscriptions.slice(0, displayedRecordsCountForExpired + recordsPerPageForExp);
...
function fillDepartmentTable(data) {
    ...
    recordsToShow.forEach(subscriptions => {
        subscriptions.forEach(subscription => {
            ...
            const budget = subscription.DepartmentNames.Budget !== null ? subscription.DepartmentNames.Budget : ''; // Extract budget, if available
...
function setSubscriptionNearingRenTable(subscriptions) {
    ...
    // Determine the records to show based on pagination parameters
    const recordsToShow = subscriptionData.slice(0, displayedRecordsCountForRen + recordsPerPageForRen);
    recordsToShow.forEach(subscription => {
        const row = document.createElement("tr");
...
function modifyRenewalCalendarReport(subscriptions) {
    ...
    // });
    const filteredSubscriptions = filtered.filter(subscription => {
    if (!subscription.SubscriptionContractAmount || subscription.SubscriptionContractAmount.Value == null) {
...
function categorizeSubscriptionsByUrgency(subscriptions) {
  ...
  function initializeCalendar(categorizedSubscriptions) {
    ...
    const urgentDates = {
        veryUrgent: categorizedSubscriptions.veryUrgent.map(sub => formatDate(new Date(sub.SubscriptionEndDate))),
        urgent: categorizedSubscriptions.urgent.map(sub => formatDate(new Date(sub.SubscriptionEndDate))),
        notUrgent: categorizedSubscriptions.notUrgent.map(sub => formatDate(new Date(sub.SubscriptionEndDate)))
    };
    ...
    function renderCalendar(month, year, monthYearId) {
        ...
        monthYearLabel.text(new Date(year, month).toLocaleString('default', { month: 'long' }) + " " + year);
        ...
        for (let day = 1; day <= totalDays; day++) {
            ...
            const dayElement = $(`<div class="${urgencyClass}"${matchingSubscription ? ` data-popover-content="${subscriptionDetails}"` : ''}>${day}</div>`);
            content.append(dayElement); // Append the day element to the calendar content
        }

        // Initialize custom popovers for highlighted days
        $('[data-popover-content]').on('mouseenter', function () {
    ...
    function createCustomPopover(element, content) {
        ...
        const popover = $('<div class="custom-popover"><div class="popover-content">' + content + '</div></div>');
    ...
    function updateArrowVisibility() {
        ...
        const isCurrentMonth1 = currentYear1 === today.getFullYear() && currentMonth1 === today.getMonth();
        const isCurrentMonth2 = currentYear2 === today.getFullYear() && currentMonth2 === today.getMonth();
...
function mergeRecordsBymonthForDepartment() {
    ...
    monthlyDepartments.forEach(record => {
        if (record && record.DepartmentNames.Name) {
            ...
            if (mergedRecords.hasOwnProperty(departmentName)) {
                mergedRecords[departmentName].SubscriptionContractAmount.Value += record.SubscriptionContractAmount.Value;
            } else {
    ...
    if (isFilterAppliedOnFinance === true) { // if user applied any filter then based of amount filter it otherwise on load no need 
        mergedArray = mergedArray.map(innerArray =>
            innerArray.filter(subscription => {
                ...
                return contractAmount >= filtersForFinance.amount1 && contractAmount <= filtersForFinance.amount2;
            })
        );
    }
...
function aggregateSubscriptionsByName(subscriptionArray) {
    ...
    subscriptionArray.forEach(record => {
        if (record && record.SubscriptionName && record.SubscriptionContractAmount && record.SubscriptionContractAmount.Value !== undefined) {
    ...
    aggregatedArray.sort((a, b) => b.SubscriptionContractAmount.Value - a.SubscriptionContractAmount.Value);
...
function aggregateByVendorProfileAndMonth(records) {
    ...
    records.forEach(record => {
        ...
        if (VendorProfile !== null && VendorProfile !== undefined && SubscriptionContractAmount && SubscriptionStartDate) {
    ...
    monthlySubscription.forEach(record => {
        if (record && record.SubscriptionStartDate) {
            ...
            if (mergedRecords.hasOwnProperty(monthKey)) {
                mergedRecords[monthKey].SubscriptionContractAmount.Value += record.SubscriptionContractAmount.Value;
            } else {
                // Otherwise, create a new entry in mergedRecords
...
function setStandardReportCards(arraySubs) {
    ...
    const totalContractAmountFuture = arraySubs.reduce((sum, subscription) => {
        const startdate = new Date(subscription.SubscriptionStartDate);
        return (startdate >= todayForRen && startdate <= endOfYear) ? sum + (subscription.SubscriptionContractAmount?.Value || 0) : sum;
    }, 0);
...
spanElement.textContent = totalContractAmount.toLocaleString('en-US', { 
    ...
    document.getElementById('renewalcost').querySelector('span').textContent = totalContractAmountFuture.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, 
    maximumFractionDigits: 2});
